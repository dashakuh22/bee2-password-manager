...
char* PwdGenShare() {
	err_t t = rngCreate(0,0);
	if (t != ERR_OK) {
		printf("PwdGenShare 1 error_code is %t\n", t);
		return 0;
	}
	cmd_pwd_t pwd = cmdPwdCreate(64);
	size_t scount = 2;
	size_t threshold = 2;
	size_t len = 32;
	cmd_pwd_t spwd = cmdPwdCreate(8);
	char* shares[] = {
		"abc",
		"cde"
	};
	t = cmdPwdGenShare_internal(&pwd, scount, threshold, len, shares, spwd);
	printf("PwdGenShare pwd = %s\n", pwd, spwd);
	printf("PwdGenShare share[0] = %s, share[1] = %s\n", shares[0], shares[1]);
	printf("PwdGenShare 2 error_code is %t = %d\n", t);
	
	FILE *fptr = fopen("gen_password", "w");
	fprintf(fptr,"%s", pwd);
	fclose(fptr);
...
char* PwdReadShare() {
	err_t t = rngCreate(0,0);
	if (t != ERR_OK) {
		printf("PwdReadShare 1 error_code is %t\n", t);
		return 0;
	}
	cmd_pwd_t pwd = cmdPwdCreate(64);
	size_t scount = 2;
	...
	t = cmdPwdReadShare_internal(&pwd, scount, len, shares, spwd);
	printf("PwdReadShare pwd = %s\n", pwd, spwd);
	printf("PwdReadShare share[0] = %s, share[1] = %s\n", shares[0], shares[1]);
	printf("PwdReadShare 2 error_code is %t = %d\n", t);
	
	FILE *fptr = fopen("read_password", "w");
	fprintf(fptr,"%s", pwd);
	fclose(fptr);
